...
path = os.path.join('log_Inception', 'InceptionV3_ImageSize_299_MaskedImage_area10000_aug2(1e-5)_weightdecay(1)', 'checkpoint30.pth')
...
with torch.no_grad():
    for index in range(len(test)):
        ...
        logit_test = model(image.view(1,3,299,299))
        # end = time.time()
...
test['LOGIT'] = logit
test.to_csv(os.path.join('Result_valid', 'InceptionV3_checkpoint30.csv'), index=False)
...
plt.title('ROC of Inception(stage 0-2 vs 3-4)')
plt.show()
